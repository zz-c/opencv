...
int main(int argc, char** argv) {
	src = imread("G:/project/c/opencv/opencv03/source/case2.png", IMREAD_GRAYSCALE);
	if (src.empty()) {
	...
	Rect roi = Rect(10, 10, src.cols - 20, src.rows - 20);//src.cols - 20, src.rows - 20
	roiImage = src(roi);
...
void detectLines(int, void*) {
	...
	vector<Vec4i> lines;
	HoughLinesP(dst, lines, 1, CV_PI / 180.0, 30, 30.0, 0);//Ö±Ïß¼ì²â
