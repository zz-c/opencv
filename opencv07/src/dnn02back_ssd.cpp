...
String model_text_file = "E:/clib/opencv-dnn/MobileNetSSD_deploy.prototxt";
String modelFile = "E:/clib/opencv-dnn/MobileNetSSD_deploy.caffemodel";
...
int main(int argc, char** argv) {
	Mat frame = imread("G:/project/c/opencv/opencv07/source/person.png");
	if (frame.empty()) {
	...
	for (int i = 0; i < detectionMat.rows; i++) {
		...
		if (confidence > confidence_threshold) {
			...
			//putText(frame, format("%s", objIndex), Point(tl_x, tl_y), FONT_HERSHEY_SIMPLEX, 1.0, Scalar(255, 0, 0), 2);
		}
	}
